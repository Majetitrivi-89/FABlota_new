  ...
  useEffect(() => {
    if (user && userRole) {
      if (userRole === "manufacturer") {
        const from = location.state?.from?.pathname || "/manufacturer";
        ...
        <div className="container mx-auto max-w-md">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground mt-2">Manufacturer Portal</p>
            </div>

            <Card variant="glass" className="border-border/50">
              <CardHeader className="pb-4">
                <CardTitle className="text-2xl text-center">Manufacturer Login</CardTitle>
                <CardDescription className="text-center">Sign in to manage your factory and products</CardDescription>
              </CardHeader>
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button type="submit" variant="hero" className="w-full" disabled={isLoading}>
                    {isLoading ? (
                      ...
                      <>Sign In as Manufacturer <ArrowRight className="h-4 w-4" /></>
                    )}
                  </Button>
                </form>
                ...
                <div className="mt-6 text-center">
                  <p className="text-sm text-muted-foreground">
                    Don't have an account? <Link to="/manufacturer/signup" className="text-primary font-medium hover:underline">Create Account</Link>
                  </p>
                </div>
              </CardContent>
            </Card>
